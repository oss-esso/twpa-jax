...
DEFAULT_HARMONIA_ETHZ_JTL_LINEAR_PARAMETER_NAMES = [
    ...
    "section_pitch",
    "section_phase",
    "short_section_segments",
    "long_section_segments",
    "long_section_every",
    "Cg_F",
    "Lj_H",
    "Cj_F",
...
def build_harmonia_ethz_jtl_linear_dataset(
    *,
    registry_csv: str | Path,
    output_dir: str | Path,
    parameter_names: Iterable[str] = DEFAULT_HARMONIA_ETHZ_JTL_LINEAR_PARAMETER_NAMES,
    require_pass: bool = True,
) -> BuiltDataset:
    """
    Build an ML-ready dataset from a bounded Harmonia ETHZ JTL linear JC campaign.

    Expected simulation type:
        harmonia_ethz_jtl_linear_jc_smoke
    ...
    selected_rows = []
    for row in rows:
        if row.get("simulation_type") != "harmonia_ethz_jtl_linear_jc_smoke":
            continue
        if require_pass and row.get("status") != "PASS":
            continue
        selected_rows.append(row)
    ...
    if not selected_rows:
        raise ValueError(
            f"No usable harmonia_ethz_jtl_linear_jc_smoke runs found in registry: {registry_csv}"
        )
    ...
    for row in selected_rows:
        ...
        if data.status.simulation_type != "harmonia_ethz_jtl_linear_jc_smoke":
            raise ValueError(
                f"Expected harmonia_ethz_jtl_linear_jc_smoke run, got "
                f"{data.status.simulation_type}: {run_dir}"
            )
    ...
    dataset_npz = output_dir / "harmonia_ethz_jtl_linear_dataset.npz"
    summary_json = output_dir / "dataset_summary.json"

    np.savez_compressed(
        dataset_npz,
        schema_version=np.asarray(SCHEMA_VERSION),
        dataset_type=np.asarray("harmonia_ethz_jtl_linear_jc_smoke"),
        parameter_names=np.asarray(parameter_names),
        parameters=parameters,
        frequency_hz=frequency_reference,
        s_real=s_complex.real,
    ...
    summary = {
        "schema_version": SCHEMA_VERSION,
        "dataset_type": "harmonia_ethz_jtl_linear_jc_smoke",
        "registry_csv": str(registry_csv),
        "dataset_npz": str(dataset_npz),
        "n_samples": int(parameters.shape[0]),
        "n_parameters": int(parameters.shape[1]),
