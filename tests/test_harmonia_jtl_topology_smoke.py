...
def test_actual_harmonia_jtl_topology_smoke_if_available(tmp_path: Path) -> None:
    ...
    with h5py.File(h5_path, "r") as h5:
        assert decode_h5_string(h5.attrs["simulation_type"]) == "harmonia_jtl_topology_smoke"
        assert decode_h5_string(h5.attrs["backend"]) == "Harmonia.CircuitIR + Harmonia.add_jtl_chain!"
        ...
        summary = json.loads(decode_h5_string(h5["topology"]["summary_json"][()]))
        circuit = json.loads(decode_h5_string(h5["topology"]["circuit_json"][()]))
