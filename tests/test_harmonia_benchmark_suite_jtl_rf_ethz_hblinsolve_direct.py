...
def test_benchmark_suite_jtl_rf_ethz_hblinsolve_direct_smoke() -> None:
    ...
    tmp_root = Path(tempfile.mkdtemp(prefix="benchmark_jtl_rf_ethz_hblinsolve_direct_"))
    ...
    try:
        ...
        cmd = [
            ...
            "--jtl-linear-backend",
            "hblinsolve_direct",
            "--rf-jtl-linear-backend",
            "hblinsolve_direct",
            "--ethz-jtl-linear-backend",
            "hblinsolve_direct",
            "--force",
        ]
        ...
        assert summary["rf_jtl_linear_backend"] == "hblinsolve_direct"
        assert summary["ethz_jtl_linear_backend"] == "hblinsolve_direct"

        expected = {
            "jtl_linear": "jtl_hblinsolve_direct",
            "rf_jtl_linear": "rf_jtl_hblinsolve_direct",
            "ethz_jtl_linear": "ethz_jtl_hblinsolve_direct",
        }

        for case_name, integration in expected.items():
            status_files = list((benchmark_dir / "runs").glob(f"{case_name}*/status.json"))
            assert status_files
            ...
            for status_path in status_files:
                ...
                assert status["cache_telemetry"]["setup_cache_integration"] == integration
    finally:
