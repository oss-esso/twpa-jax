...
from pump_solvers.schur_partition import build_partition, assemble_schur_complements  # noqa: E402
...
def assemble_conversion_matrix_from_base(
    ...
    dblocks = [dynamic_block(ipm, omega_s + m * omega_p, loss_model=loss_model) for m in ms]
...
def solve_gain_one_schur(
    ...
    t0 = time.perf_counter()
    linear_blocks = [dynamic_block(ipm, omega_s + m * omega_p, loss_model=loss_model) for m in ms]
    part = build_partition(linear_blocks, ipm.Bphi, [source_index, out_index])
