...
backup = path.with_suffix(".py.before_spectral_jvp_v2")
...
# 3. Add setting field.
# -------------------------------------------------------------------------
if "    jvp_mode: str" not in text:
    old = """    continuation_predictor: str"""
    new = """    continuation_predictor: str
    jvp_mode: str"""
    if old not in text:
        raise SystemExit("Could not find continuation_predictor setting field.")
    text = text.replace(old, new)

# -------------------------------------------------------------------------
# 4. Add CLI flag robustly.
# -------------------------------------------------------------------------
if "--jvp-mode" not in text:
    old = """    p.add_argument("--min-alpha", type=float, default=1.0 / 1024.0)"""
    new = """    p.add_argument("--jvp-mode", choices=["aft", "spectral"], default="aft")
    p.add_argument("--min-alpha", type=float, default=1.0 / 1024.0)"""
    if old not in text:
        raise SystemExit("Could not find --min-alpha insertion point.")
    text = text.replace(old, new)

# -------------------------------------------------------------------------
# 5. Pass jvp_mode into settings.
# -------------------------------------------------------------------------
if "jvp_mode=args.jvp_mode" not in text:
    ...
    if old not in text:
        raise SystemExit("Could not find settings construction block.")
    text = text.replace(old, new)

# -------------------------------------------------------------------------
# 6. Print and metadata.
# -------------------------------------------------------------------------
if 'print(f"jvp_mode={args.jvp_mode}")' not in text:
    ...
    if old not in text:
        raise SystemExit("Could not find preconditioner print line.")
    text = text.replace(old, new)
...
if '"jvp_mode": args.jvp_mode' not in text:
    ...
    new = """        "preconditioner": args.preconditioner,
        "jvp_mode": args.jvp_mode,"""
    if old not in text:
        raise SystemExit("Could not find preconditioner metadata line.")
    text = text.replace(old, new)

# -------------------------------------------------------------------------
# 7. Build spectral tangent once per Newton step and use it in matvec.
# -------------------------------------------------------------------------
if "spectral_tangent = problem.spectral_tangent_state(tangent)" not in text:
...
if "jvp_coeffs_with_spectral_tangent" in text and 'if s.jvp_mode == "spectral":' not in text:
