...
BASE_OUT = ROOT / "outputs" / "exp14_fxjtwpa_sideband_scan"
...
def load_jc(path: Path):
    out = {}
    with path.open(encoding="utf-8") as f:
        for r in csv.DictReader(f):
            out[round(float(r["signal_ghz"]), 6)] = float(r["gain_db"])
    return out
...
def load_py(path: Path):
    ...
    with path.open(encoding="utf-8") as f:
        for r in csv.DictReader(f):
            out[round(float(r["signal_ghz"]), 6)] = r
    return out

def compare(py_csv: Path):
    py = load_py(py_csv)
    jc = load_jc(JC)
...
for sidebands in [4, 6, 8, 10, 12]:
    outdir = BASE_OUT / f"sidebands_{sidebands}"
    ...
    for r in bests:
        row = {"sidebands": sidebands, **r}
        all_rows.append(row)

    print("BEST", sidebands, bests[0])
