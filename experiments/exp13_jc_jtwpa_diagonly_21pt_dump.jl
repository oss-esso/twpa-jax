...
outdir = raw"D:\Projects\Thesis\twpa_jax\outputs\exp13_jtwpa_diagonly"
mkpath(outdir)
...
# Pump is still present, but signal/idler sideband coupling is disabled.
Nmodulationharmonics_diagonly = (0,)

println("=== running JC JTWPA pumped diagonal-only 21-point curve ===")

timed = @timed JosephsonCircuits.hbsolve(
    ws,
    artifacts["wp"],
    artifacts["sources"],
    Nmodulationharmonics_diagonly,
...
out_csv = joinpath(outdir, "jc_jtwpa_diagonly_curve_21pt.csv")
