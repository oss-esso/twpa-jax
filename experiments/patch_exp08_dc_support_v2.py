...
backup = path.with_suffix(".py.before_dc_support_v2")
if not backup.exists():
    backup.write_text(text, encoding="utf-8")

# ---------------------------------------------------------------------
# Add DC loader helper before class HarmonicPumpProblem.
# ---------------------------------------------------------------------
if "def load_dc_solution(" not in text:
    m = re.search(r"(?m)^class\s+HarmonicPumpProblem\b|^@dataclass.*\nclass\s+HarmonicPumpProblem\b", text)
    if not m:
        m = re.search(r"(?m)^.*class\s+HarmonicPumpProblem\b.*$", text)
    if not m:
        print("Could not find HarmonicPumpProblem. Nearby class names:")
        for mm in re.finditer(r"(?m)^class\s+\w+", text):
            print(mm.group(0))
...
# ---------------------------------------------------------------------
# Initialize DC branch flux after self.nb.
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Shift nonlinear pump current by DC branch flux.
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Shift tangent gamma by DC branch flux.
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Load DC solution before HarmonicPumpProblem construction.
# ---------------------------------------------------------------------
if "dc_x, dc_branch_flux = load_dc_solution(args.dc_solution, ipm)" not in text:
    old = "    problem = HarmonicPumpProblem(\n"
...
# ---------------------------------------------------------------------
# Put DC metadata into pump_report before write_results.
# ---------------------------------------------------------------------
