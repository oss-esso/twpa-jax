...
def make_harmonia_ethz_jtl_linear_config(
    *,
    index: int,
    Lj_H: float,
    n_frequency: int = 5,
    ...
    return {
        ...
        "simulation_type": "harmonia_ethz_jtl_linear_jc_smoke",
        "circuit_template": "circuit_ir_ethz_jtl_chain_linear_jc",
        "seed": 6301 + index,
        "parameters": {
            "prefix": "ethz",
            "start_node": "n1",
        ...
        "solver": {
            ...
            "notes": "Tiny bounded ETHZ JTL linear campaign over Lj_H.",
        },
    }
...
def compute_ethz_jtl_linear_metrics(run_dir: Path) -> dict[str, Any]:
...
def run_campaign(
    *,
    ...
    for idx, lj_h in enumerate(lj_values_h):
        ...
        config = make_harmonia_ethz_jtl_linear_config(
            index=idx,
            Lj_H=float(lj_h),
        ...
        if result.ok:
            run_record["metrics"] = compute_ethz_jtl_linear_metrics(output_dir)
        else:
    ...
    summary = {
        "schema_version": SCHEMA_VERSION,
        "campaign_type": "harmonia_ethz_jtl_linear_lj_sweep",
...
def print_human_summary(summary: dict[str, Any]) -> None:
    ...
    print("Harmonia ETHZ JTL linear campaign")
    print("=================================")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        nargs="+",
        default=[140.0e-12, 158.0e-12, 180.0e-12],
    )
    parser.add_argument(
        "--harmonia-root",
        type=Path,
    ...
    parser.add_argument(
        "--campaign-dir",
        type=Path,
        default=_WORKSPACE_ROOT / "outputs" / "campaigns" / "harmonia_ethz_jtl_linear_jc",
    )
