...
def _run_point(cells: int, signal_ghz: float, power_dbm: float, sidebands: int, external_flux_on_small_junction: bool = False, pump_power_dbm: float = -78.4, pump_modes: tuple[int, ...] = (1, 3, 5)) -> dict[str, object]:
    ...
    return {
        ...
        "s21_on_abs": float(abs(s21)),
        "s21_off_abs": float(abs(s21_off)),
        ...
        "resolved_signal_GHz": float(basis.signal_tone.omega(omega_p, delta) / (2.0 * math.pi * 1e9)),
        "resolved_idler_GHz": float(basis.idler_tone.omega(omega_p, delta) / (2.0 * math.pi * 1e9)),
        "n_tones": int(basis.n_tones), "n_p": int(basis.n_p or 0), "n_delta": int(basis.n_delta or 0),
