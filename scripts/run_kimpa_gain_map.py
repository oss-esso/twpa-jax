...
from twpa_solver.core import kinetic_dc_branch_flux, PortEnvironment
...
def _run_best_spectrum(args: argparse.Namespace, run_dir: Path, best: dict[str, object]) -> None:
    ...
    for freq in freqs:
        result = solve_gain_one(
            ...
            source_port=1, out_port=1, z0_ohm=50.0, environment=environment,
        )
        s11 = port_s_from_unit_current_response(result.vout_on, source_port=1, out_port=1, z0_ohm=50.0)
        rows.append({"signal_ghz": float(freq), "idler_ghz": pump_freq - float(freq), "gain_db": result.gain_db, "gain_vs_off_db": result.gain_vs_off_db, "s11_real": float(np.real(s11)), "s11_imag": float(np.imag(s11)), "s11_phase_deg": float(np.angle(s11, deg=True)), "idler_power_rel_to_signal_off_db": result.idler_power_rel_to_signal_off_db, "status": result.status, "linear_rel_residual": result.linear_rel_residual})
    ...
    ax.plot(freqs, gain_values, label="reflection gain")
    ...
    plt.close(fig)
    summary = {"pump_dbm_internal": pump_dbm, "signal_best_ghz": signal_best, "idler_best_ghz": pump_freq - signal_best, "max_current_over_ic": best["max_current_over_ic"], "peak_spectrum_gain_db": float(np.nanmax(gain_values)), "peak_spectrum_signal_ghz": float(freqs[np.nanargmax(gain_values)]), "points": args.spectrum_points, "start_ghz": args.spectrum_start_ghz, "stop_ghz": args.spectrum_stop_ghz}
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    metadata = {"fixture": args.fixture, "pump_ghz": args.pump_ghz, "degenerate_signal_ghz": args.pump_ghz / 2.0, "dc_current_a": args.dc_current_a, "environment": args.environment, "pump_axis_note": "Plots use peak total current / Ic; internal dBm is metadata only.", "idler_definition": "physical idler frequency = pump frequency - signal frequency; Floquet idler_m=-1"}
